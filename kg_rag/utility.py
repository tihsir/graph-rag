...
# PROJECT CODE 
def gemini_call_mode5(prompt, system_prompt, temperature=0.0):
    """
    A Gemini wrapper ONLY for Mode 5.
    Ensures the system prompt is actually applied (Gemini requires role messages).
    Does NOT affect modes 0–4.
    """
    try:
        ...
        return f"ERROR_MODE5_GEMINI: {str(e)}"
###############################################################
...
def load_semantic_db(index_path, chunks_path):
    # Load FAISS index
    index = faiss.read_index(index_path)
    
    # Load chunks (text array/list)
...
def semantic_retrieve(question, index, chunks, embed_model, k):
    # Embed the question
